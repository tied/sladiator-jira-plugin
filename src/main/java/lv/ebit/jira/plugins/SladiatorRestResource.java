...
public class SladiatorRestResource {
    ...
    private boolean isAuthorized(HttpServletRequest request, String projectId) {
        ...
        return project.getProjectLead().getName().equals(user.getName()) || permissionManager.hasPermission(new ProjectPermissionKey("23"), project, user);
    }

    private boolean isAdmin(HttpServletRequest request) {
        String username = userManager.getRemoteUsername(request);
        return username != null && (username == null || userManager.isSystemAdmin(username));
    }
