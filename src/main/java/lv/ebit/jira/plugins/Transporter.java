...
public class Transporter implements Runnable {
	...
	public void run() {
		...
		if (slaTokens.size() > 0) {
			for (int i = 0; i < slaTokens.size(); i++) {
				...
				String output = collectIssueInfo(url, slaTokens.get(i), issue);
				if (eventTypeId.equals(EventType.ISSUE_CREATED_ID)) {
					PostMethod httpMethod = new PostMethod(realSlaUrl);
					httpMethod.setRequestHeader("Content-Type", "application/json");
					httpMethod.setRequestHeader("SLA_TOKEN", slaTokens.get(i));
					try {
						httpMethod.setRequestEntity(new StringRequestEntity(output, "application/json", null));
					...
					DeleteMethod httpMethod = new DeleteMethod(realSlaUrl+issue.getKey());
					httpMethod.setRequestHeader("Content-Type", "application/json");
					httpMethod.setRequestHeader("SLA_TOKEN", slaTokens.get(i));
					try {
						int statusCode = client.executeMethod(httpMethod);
					...
					PutMethod httpMethod = new PutMethod(realSlaUrl+issue.getKey());
					httpMethod.setRequestHeader("Content-Type", "application/json");
					httpMethod.setRequestHeader("SLA_TOKEN", slaTokens.get(i));
					try {
						httpMethod.setRequestEntity(new StringRequestEntity(output, "application/json", null));
	...
	public String collectIssueInfo(String url, String slaToken, Issue issue) {
...
//			json.putOpt("components", components);
			...
			if (configuration.getConfig().get(slaToken).isSendAssignee()) {
	...
	public List<JSONObject> getTransitions(Issue issue) throws JSONException {
		...
		for (GenericValue changeGroup : changeGroups) {
			Map<String, ? extends Object> paramsItem = MapBuilder.build(
